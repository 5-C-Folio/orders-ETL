SELECT 'HC' AS poNumberPrefix,
    ...
    rr.Z13_TITLE,
    rr.author,
    ord.z68_isbn --ord.z68_material_type
...
FROM HAM50.Z68 ord
    LEFT JOIN (
        SELECT brief.Z13_REC_KEY AS z13key,
            brief.Z13_TITLE,
            brief.Z13_author AS author,
            SUBSTR(lkr.Z103_REC_KEY, 6, 9) AS ADM_N
        FROM HAM50.Z103 lkr
            INNER JOIN FCL01.z13 brief ON SUBSTR(brief.Z13_REC_KEY, 0, 9) = SUBSTR(lkr.Z103_REC_KEY_1, 6, 9)
            AND lkr.Z103_LKR_TYPE = 'ADM'
            AND SUBSTR(lkr.Z103_REC_KEY_1, 1, 5) = 'FCL01'
    ) rr ON SUBSTR(ord.Z68_REC_KEY, 0, 9) = rr.ADM_N
    left JOIN (
        SELECT ordnum,
            SUBSTR(Z601_REC_KEY, 0, 50) AS fundName
        FROM (
                SELECT Z601_REC_KEY,
                    SUBSTR(Z601_REC_KEY_3, 0, 9) AS ordnum,
                    max(SUBSTR(Z601_REC_KEY, 50, 25)) over (partition by SUBSTR(Z601_REC_KEY_3, 0, 9)) AS max_open_date
                FROM HAM50.Z601
                WHERE SUBSTR(Z601_REC_KEY_3, 0, 9) <> '000000000'
            )
        WHERE max_open_date = SUBSTR(Z601_REC_KEY, 50, 25)
    ) budName ON rr.ADM_N = budname.ordnum
WHERE
